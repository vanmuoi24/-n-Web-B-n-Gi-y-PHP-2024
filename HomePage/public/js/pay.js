...
function loadPayPage() {
								...
								<div class="pay-right">
									<div class="pr-order">
										<div class="pr-wrapper">
											...
											<div class="pr-final-total-wrapper">
	...
	`;
		...
		loadInfoInPay();
		loadProductInPay();
		handleCheckPromotion(formatDate(new Date()));
		hideMyCart();
...
function renderTotal(ship = 0) {
	...
	if (ship !== 0) {
		...
		feeShip = totalPrice >= 3999000 ? 0 : 35000;
	...
	document.querySelector('.pr-final-total-wrapper').innerHTML = `
		<p class="mcr-total">Tổng tiền</p>
		<p class="mcr-total-value">${formatCurrency(totalPrice + feeShip)}</p>
	`;
}
function renderShip(status) {
	if (!status) {
		document.querySelector('.pr-ship-wrapper').innerHTML = `
				<p class="pr-ship">Giao hàng</p>
				<p class="pr-ship-value">Tiêu chuẩn (3-4 ngày) <br> ${formatCurrency(
...
function handleCheckPromotion(payDate) {
	var xhr = new XMLHttpRequest();
	xhr.open(
		'POST',
		`../../mvc/API/index.php?type=checkPromotion&date=${payDate}`,
		true
	);
	xhr.setRequestHeader('Content-type', 'application/json');
	xhr.onreadystatechange = function () {
